...
1. Clone repo and install [requirements.txt](requirements.txt) in a python=3.10 environment
   ...
   cd Custom-Algorithm/01_ModelTraining/03_Segmentation/Yolov11-seg
   ```
...
```
model: yolo11n-seg.pt  # 将 yolo11n-seg.pt 修改为需要转换的模型路径
```
...
# 如果待转换模型是 "yolo11n-seg.pt"， 执行后生成 "yolo11n-seg.onnx" 模型。
```
