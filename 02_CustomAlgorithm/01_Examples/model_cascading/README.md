...
- **算法名称修改**。修改为自己定义的算法名称，如：custom_fire，若不修改，会覆盖已有算法文件。

![](../../../docs/assets/algname_fire.png)

- **模型名称修改**。修改为自定义的模型名称，如custom_fire，若不修改，会覆盖已有模型。

![](../../../docs/assets/modelname_fire.png)

- **模型文件修改**。量化后的模型文件统一命名为model，如下图所示，不可命名为model.rknn。

![](../../../docs/assets/model_fire.png)

- **模型类型修改**。模型类型为推理代码文件名称，修改为自定义名称。如custom_detect.py。

<img src="../../../docs\assets\model_type_fire.png" style="zoom:150%;" />
...
若[转换模型](../../../01_ModelTraining/01_Detection/Yolov5/README.md)的anchors不是通用anchors，需在args下增加anchors参数。
    ...
    infer_time: 60
    ```
![](../../../docs/assets/model_yaml_fire.png)

- **postprocessor.yaml文件修改**。第1行是算法名称，第2行是算法中文名称，第3行是算法描述，第4行是算法组类别，第6行至第19行是模型参数，第20行是告警label。

<img src="../../../docs/assets/postprocessor_yaml_fire.png" style="zoom:150%;" />

- 前端配置文件xxx.json修改。详细可参照[前端配置说明](../../03_PackageStructure/README.md)。修改模型名称、算法名称、语音播报内容、置信度label等。

![](../../../docs/assets/alg_json_fire.png)
