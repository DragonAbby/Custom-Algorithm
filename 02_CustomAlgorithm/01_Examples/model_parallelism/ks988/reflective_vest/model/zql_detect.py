    ...
    def infer(self, data, **kwargs):
        ...
        if self.status:
            try:
                ...
                data, dw, dh = self._letterbox(data, (self.img_size, self.img_size))
                outputs = self._onnx_infer('model', data)
                # Do postprocess
                input_data = list()
                for i in range(len(outputs)):
                    input_data_ = outputs[i].reshape([3, -1] + list(outputs[i].shape[-2:]))
                    input_data.append(np.transpose(input_data_, (2, 3, 0, 1)))
                boxes, classes, scores = self.__yolov5_post_process(input_data)
                if boxes is not None:
                    for j, box in enumerate(boxes):
                        obj = {
                            'label': int(classes[j]),
                            'conf': round(float(scores[j]), 2)
                        }
                        box = boxes[j]
