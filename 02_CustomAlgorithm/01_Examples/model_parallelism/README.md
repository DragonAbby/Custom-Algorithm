...
- 如果产品型号为ks968，则在ks968下的[reflective_vest](../../02_Demo/AlgorithmPackage/ks968/reflective_vest)算法包上修改。
- 如果产品型号为ks988，则在ks988下的[reflective_vest](../../02_Demo/AlgorithmPackage/ks988/reflective_vest)算法包上修改。
...
- **算法名称修改**。修改为自己定义的算法名称，如：custom_vest，若不修改，会覆盖已有算法文件。

![](../../../docs/assets/algname_vest.png)

- **模型名称修改**。修改为自定义的模型名称，如custom_vest，若不修改，会覆盖已有模型。

![](../../../docs/assets/modelname_vest.png)

- **模型文件修改**。量化后的模型文件统一命名为model，如下图所示，不可命名为model.rknn。

![](../../../docs/assets/model_vest.png)

- **模型类型修改**。模型类型为推理代码文件名称，修改为自定义名称。如custom_detect.py。

<img src="../../../docs/assets/model_type_vest.png" style="zoom:150%;" />

- **model.yaml文件修改**。第1行，第8行是模型名称，第2行，第9行模型类型是推理代码的名称，第4行-第6行，第10行-第13行是模型输入参数。第7行，第13行是模型推理时间，其设置应当保证source队列没有积压，队列存在积压，则增加推理时间。  
若[转换模型](../../../01_ModelTraining/01_Detection/Yolov5/README.md)的anchors不是通用anchors，需在args下增加anchors参数。
    ...
    infer_time: 60
    ```
![](../../../docs/assets/model_yaml_vest.png)

- **postprocessor.yaml文件修改**。第1行是算法名称，第2行是算法中文名称，第3行是算法描述，第4行是算法组类别，第6行至第15行是模型参数，第16行是告警label。

<img src="../../../docs/assets/postprocessor_yaml_vest.png" style="zoom:150%;" />

- **前端配置文件xxx.json修改**。详细可参照[前端配置说明](../../03_PackageStructure/README.md)。修改模型名称、算法名称、语音播报内容、置信度label等。

![](../../../docs/assets/alg_json_vest.png)
...
推理与后处理代码位于[DevelopmentKit](../../02_Demo/DevelopmentKit)文件夹下。下图为模块调用流程图。
