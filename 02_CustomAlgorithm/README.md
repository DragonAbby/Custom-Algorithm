...
- AlgorithmPackage: 包含ks968和ks988两种型号，39种标配算法包样例，可供参考。
...
## 自定义算法包流程
### 1. 确定任务类型
- 根据业务需求，参考01_Examples，选择一种参考样例。
### 2. 推理代码实现
- 若自定义算法包所需模型在01_ModelTraining提供的模型样例中，则无需编写推理代码。
- 若所用模型未在01_ModelTraining中提供，参考02_Demo/DevelopmentKit/model编写推理代码。
### 3. 后处理代码实现
- 根据参考样例，调整后处理代码逻辑。
### 4. 算法包配置
- 参考03_PackageStructure中算法包参数详细说明，修改自定义算法包配置文件参数。
### 5. 算法包加密
- 使用Tools/ks-tools对算法包加密，生成bin文件。
### 6. 导入算法仓库、添加算法、调试
