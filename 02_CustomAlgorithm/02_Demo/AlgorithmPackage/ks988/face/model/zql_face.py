...
class Model(OnnxModel):
    ...
    def __init__(self, acc_id, name, conf):
        ...
        self.det_mean = [127.5, 127.5, 127.5]
        self.det_std = [128.0, 128.0, 128.0]
        self.quality_mean = [127.5, 127.5, 127.5]
        self.quality_std = [127.5, 127.5, 127.5]
    ...
    def __infer_det(self, image):
        ...
        image, dw, dh = self._letterbox(image, (self.img_size, self.img_size), mean=self.det_mean, std=self.det_std)
        outputs = self._onnx_infer('det', image)
        # Do postprocess
        bbox_list, score_list, kps_list = self.__post_process(outputs, dw, dh, (self.img_size, self.img_size, 3))
        if not bbox_list:
            return None
    ...
    def __face_alignment(self, image, boxes, landmarks):
        ...
        for i, src_pts in enumerate(landmarks):
            ...
            face_warped.append((face_img - self.quality_mean) / self.quality_std)
        return face_warped
    ...
    def __extract_feature(self, face_warped):
        ...
        for image in face_warped:
            ...
            outputs = self._onnx_infer('rec', image)
            output = outputs[0]
    ...
    def __infer_quality(self, face_warped):
        ...
        for image in face_warped:
            image = np.transpose(image, [2, 0, 1])
            image = np.expand_dims(image, axis=0).astype(np.float32)
            outputs = self._onnx_infer('quality', image)
            face_quality.append(outputs[0][0][0])
        return face_quality
    ...
    def infer(self, data, **kwargs):
        ...
        if self.status:
            try:
                ret = self.__infer_det(data)
                if ret is not None:
                    boxes, scores, landmarks = ret
                    face_warped = self.__face_alignment(data, boxes, landmarks)
                    ...
                    for zip_ in zip(boxes, scores, face_quality, face_features):
                        ...
                        obj['xyxy'] = [xyxy[0] if xyxy[0] >= 0 else 0,
                                       ...
                                       xyxy[2] if xyxy[2] <= data.shape[1] else data.shape[1],
                                       xyxy[3] if xyxy[3] <= data.shape[0] else data.shape[1]]
