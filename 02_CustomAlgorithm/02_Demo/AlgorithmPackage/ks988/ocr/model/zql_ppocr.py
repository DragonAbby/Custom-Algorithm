...
class Model(OnnxModel):
    ...
    def __init__(self, acc_id, name, conf):
        ...
        self.postprocess_op = process_pred(os.path.join(conf['path'], 'ppocr_keys_v1.txt'), 'ch', True)
    ...
    def __infer_det(self, image):
        ...
        try:
            ...
            outputs = self._onnx_infer('ppdet', img)
            # 后处理
    ...
    def get_img_res(self, img, process_op):
        ...
        img = img[np.newaxis, :]
        outputs = self._onnx_infer('pprec', img)
    ...
    def infer(self, data, **kwargs):
        ...
        if self.status:
            try:
                dt_boxes, dt_confs = self.__infer_det(data)
                if dt_boxes:
                    for bbox, det_conf in zip(dt_boxes, dt_confs):
                        rec_info = self.__infer_rec(data, bbox)
                        obj = {
                            'xyxy': bbox.tolist(),
