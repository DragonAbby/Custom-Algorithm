...
class Model(OnnxModel):
    ...
    def __init__(self, acc_id, name, conf):
        ...
        self.rec_mean = [149.94, 149.94, 149.94]
        self.rec_std = [49.215, 49.215, 49.215]
    ...
    def __infer_rec(self, layer_num, pad):
        ...
        data = pad[:, :, ::-1]
        data, _, _ = self._letterbox(data, self.rec_size, mean=self.rec_mean, std=self.rec_std, stretch=True)
        outputs = self._onnx_infer('rec', data)
        if outputs:
