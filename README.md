...
- 模型训练过程：参考 [**01_ModelTraining**](./01_ModelTraining) 完成数据准备、模型训练、导出 ONNX 格式、模型量化步骤。
- 算法定义过程：参考 [**02_CustomAlgorithm**](./02_CustomAlgorithm) 完成推理代码、后处理代码、配置文件编写，完成算法包构建。
...
- [01_Examples](./02_CustomAlgorithm/01_Examples): 多种算法逻辑实现样例
    - 提供不同功能算法的代码示例，帮助用户根据具体需求进行算法定制。
- [02_Demo](./02_CustomAlgorithm/02_Demo): 晓知精灵标配算法包、开发套件参考样例
...
- **参考内容**：
    ...
    - 按照以下步骤制作算法包：
        ...
        - 推理部分：根据使用的模型类型，选择对应的[推理代码](./02_CustomAlgorithm/02_Demo/DevelopmentKit/model)。
        - 后处理部分：根据任务种类，参考[**开发样例**](./02_CustomAlgorithm/02_Demo/DevelopmentKit/postprocessor)，实现算法功能。
        - 算法包加密：使用 Tools 目录中的[加密工具](./Tools/ks-tools/ks-tools.zip)，对算法包进行加密，保障模型和代码的安全性。
    - 算法包完成后，可以将其部署到目标平台。

### 3. 部署与测试
- **目标**：将算法包部署到目标设备，并进行功能测试，确保算法包制作正确。
- **参考内容**：
    - 调试与日志：在 [02_CustomAlgorithm/DebugAndLogs](./02_CustomAlgorithm/04_DebugAndLogs) 目录下，查阅调试方法和日志查看指南，帮助您快速定位部署中的问题。
